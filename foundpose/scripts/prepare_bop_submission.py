...
from foundpose.utils import misc, logging, json_util
...
if object_lids is None:
    ...
    bop_model_props = dataset_params.get_model_params(
        datasets_path=datasets_path, dataset_name=object_dataset
    )
...
for object_lid in object_lids:
    ...
    for scene_id, img_id in run_time_per_image.keys():
        total_run_time[(scene_id, img_id)] = (
            run_time_per_image[(scene_id, img_id)]
            + detection_time_per_image[(scene_id, img_id)]
        )
