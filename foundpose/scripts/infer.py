...
"""Do not use ground truth for time optimization. """
...
from foundpose.utils.misc import array_to_tensor, tensor_to_array, tensors_to_arrays
...
from foundpose.utils.misc import warp_depth_image, warp_image
...
def infer(opts: InferOpts) -> None:
    ...
    test_targets_path = os.path.join(
        bop_test_split_props["base_path"], "test_targets_bop19.json"
    )
    ...
    eval_modality, eval_sensor = (
        bop_test_split_props["eval_modality"],
        bop_test_split_props["eval_sensor"],
    )
        ...
        if eval_modality is not None and eval_sensor is not None:
            scene_cameras[scene_id] = data_util.load_chunk_cameras(
                bop_test_split_props[
                    f"scene_camera_{eval_modality}_{eval_sensor}_tpath"
                ].format(scene_id=scene_id),
                bop_test_split_props["im_size"][eval_sensor],
            )
        else:
            scene_cameras[scene_id] = data_util.load_chunk_cameras(
                bop_test_split_props[f"scene_camera_tpath"].format(scene_id=scene_id),
    ...
    renderer = renderer_builder.build(
        renderer_type=renderer_type, model_path=bop_model_props["model_tpath"]
    )
    ...
    for object_lid in object_lids:
        ...
        models_info = inout.load_json(
            bop_model_props["models_info_path"], keys_to_int=True
        )
        ...
        for item_info in targets_per_obj[object_lid]:
            ...
            for inst_j, instance in enumerate(instances):
                ...
                for hypothesis_id, final_pose in enumerate(final_poses):
                    ...
                    coarse_trans_m2w = trans_c2w.dot(
                        misc.get_rigid_matrix(coarse_est_m2c)
                    )
...
def main() -> None:
    opts = config_util.load_opts_from_json_or_command_line(InferOpts)[0]
    infer(opts)
