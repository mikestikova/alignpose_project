...
from foundpose.utils import geometry, logging

import foundpose.utils.structs as structs
...
from foundpose.utils.geometry import transform_3d_points_numpy, gen_look_at_matrix
...
def sample_views(
    ...
    for pt in pts:
        ...
        R_yz_flip = geometry.rotation_matrix_numpy(math.pi, np.array([1, 0, 0]))[:3, :3]
        R = R_yz_flip.dot(R)
...
def get_object_bbox_in_camera(
    vertices: np.ndarray,
    pose_cm: np.ndarray,
    camera: PinholePlaneCameraModel,
) -> Optional[AlignedBox2f]:
    pts_in_camera = geometry.transform_3d_points_numpy(
        pose_cm, vertices
    )  # Points in camera coordinates, shape (N,3)
    pts_in_image = camera.eye_to_window(
        pts_in_camera
    )  # Points in image coordinates, shape (N,2)

    # Get amodal bounding box in image plane
    orig_box_amodal = AlignedBox2f(
        left=max(pts_in_image[:, 0].min(), 0),
        top=max(pts_in_image[:, 1].min(), 0),
        right=min(pts_in_image[:, 0].max(), camera.width - 1),
        bottom=min(pts_in_image[:, 1].max(), camera.height - 1),
    )
    return orig_box_amodal
...
def crop_and_resize_image(
    ...
    resized_image = resize_image(
        cropped_image, output_size, interpolation=interpolation
    )
...
def crop_image(image: np.ndarray, crop_box: AlignedBox2f) -> np.ndarray:
    ...
    Returns:
        Cropped image.
    """
    left, top, right, bottom = crop_box.int_array_ltrb()
    return image[top:bottom, left:right]


def distance_for_rendering(
    obj_radius: float,
    camera_model: CameraModel,
):
    """Calculates the distance from the object center for rendering.
    ...
    """
    render_res = min(camera_model.width, camera_model.height)
    f = 0.5 * (camera_model.f[0] + camera_model.f[1])

    r_pix = render_res / 4  # object will cover a quarter of the image height
    distance = 2 * obj_radius * f / r_pix
    return distance
...
def get_default_camera_model(render_res: int = 1024) -> PinholePlaneCameraModel:
    """Returns a default camera model for rendering.

    Args:
        render_res: The resolution (width and height) of the camera.
    Returns:
        A default pinhole camera model.
    """
    ...
    cx = render_res / 2
    cy = render_res / 2
    fx = render_res / 2.0 / math.tan(fov_rad / 2.0)
    fy = fx
    image_side = render_res

    # Prepare a camera for the template (square viewport).
