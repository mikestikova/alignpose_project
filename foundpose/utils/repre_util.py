...
from foundpose.utils import projector_util, logging, misc
from foundpose.utils.structs import PinholePlaneCameraModel

from foundpose.utils.misc import tensor_to_array
...
class FeatureBasedObjectRepre:
    ...
    # to the camera space.
    template_cameras_cam_from_model: List[PinholePlaneCameraModel] = field(
...
def save_object_repre(
    ...
    object_dict["template_desc_opts"] = (
        repre.template_desc_opts._asdict()
        if repre.template_desc_opts is not None
        else None
    )
    ...
    for projector in repre.feat_raw_projectors:
        object_dict["feat_raw_projectors"].append(
            projector_util.projector_to_tensordict(projector)
        )
