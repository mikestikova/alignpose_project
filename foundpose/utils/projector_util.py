...
from foundpose.utils.misc import array_to_tensor, tensor_to_array
...
class TorchPCAProjector(Projector):
    def __init__(
        self,
        n_components: int,
        whiten: bool = False,
    ...
    def fit_from_numpy(self, pca: PCA) -> None:
        self.pca = pca
        self.mean = array_to_tensor(pca.mean_).to(self.device)
        assert self.n_components == pca.components_.shape[0], (
            "Number of components does not match."
        )
        self.components = array_to_tensor(pca.components_).to(self.device)

    def transform_numpy(self, data_x: torch.Tensor) -> torch.Tensor:
        return array_to_tensor(self.pca.transform(tensor_to_array(data_x))).to(
            data_x.device
        )
    ...
    def transform(self, data_x: torch.Tensor) -> torch.Tensor:
        if self.whiten:
            return self.transform_numpy(data_x)
        self.mean = self.mean.to(data_x.device)
        self.components = self.components.to(data_x.device)
        data_x_centered = data_x - self.mean
        projected = torch.matmul(data_x_centered, self.components.T)
        return projected
...
def projector_from_tensordict(projector_dict: Dict[str, Any]) -> Projector:
    ...
    if "pca_projector" in projector_dict:
        ...
        pca.explained_variance_ratio_ = tensor_to_array(
