...
from foundpose.utils.misc import (
    array_to_tensor,
    arrays_to_tensors,
)
...
from foundpose.utils import json_util, logging, structs, misc
...
def prepare_sample(
    ...
    if type(split_props["im_modalities"]) is dict:
        ...
        image_path = split_props[f"{eval_modality}_{eval_sensor}_tpath"].format(
            scene_id=chunk_id, im_id=im_id
        )
    else:
        eval_modality = "rgb"
        image_path = split_props[f"{eval_modality}_tpath"].format(
            scene_id=chunk_id, im_id=im_id
        )
...
def parse_chunk_cameras(
    ...
    for im_id, camera_json in cameras_json.items():
        ...
        for k, v in camera_json.items():
            ...
            elif k == "cam_R_w2c" or k == "R_w2c":
                extrinsics_w2c[:3, :3] = np.array(v, np.float32).reshape((3, 3))
            elif k == "cam_t_w2c" or k == "t_w2c":
